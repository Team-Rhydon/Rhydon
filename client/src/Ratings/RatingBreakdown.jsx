  ...
  return (
    <div ref={ratingsRef} className="Star-Ratings">
      ...
      <div className="Star-Bars">
        {Object.keys(ratingBars).sort((a, b)=> {
          a-b; return -1;
        }).map((star, i) => {
          return (
            <div className="StarNumber"
              key={i}
              onClick={(e) => setFilter((prevFilter) => {
                prevFilter[star] = !prevFilter[star]; return {...prevFilter};
              })}>
          ...
          {isFiltered?
            <span>Displaying
              {Object.keys(filter).map((key, i) => {
                return filter[key] ? <span key={i}> {key} </span>: null;
              })}
              star reviews
